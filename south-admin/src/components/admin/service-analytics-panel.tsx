...
export default function ServiceAnalyticsPanel() {
  ...
  const stats = [
    { label: 'إجمالي الطلبات', value: totalOrders, icon: ShoppingCart, color: 'text-[#8B1E3A]', bg: 'bg-[#8B1E3A]/10' },
        ...
        <div>
          {serviceStats.slice(0, 20).map((provider, i) => (
            <div key={i} className="ios-list-item gap-3">
              <div className="w-8 h-8 rounded-xl bg-[#8B1E3A]/10 flex items-center justify-center text-xs font-bold text-[#8B1E3A] shrink-0">
                {i + 1}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="w-full bg-muted/30 rounded-full h-1.5 mt-1">
                  <div
                    className="bg-[#8B1E3A] h-1.5 rounded-full transition-all duration-500"
