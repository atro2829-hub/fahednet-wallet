...
import { formatNumber, currencySymbols, generateId, formatDateAr, timeAgo } from '@/lib/utils';
...
export default function InvestmentsPanel() {
      ...
      Object.entries(data).forEach(([uid, userData]: [string, any]) => {
        ...
        Object.entries(userInvestments).forEach(([investId, investData]: [string, any]) => {
          investList.push({
            id: investId,
            uid,
            userName: userData.name || userData.firstName || userData.email || uid,
            ...investData,
          });
        });
      });
  ...
  const handleSavePlan = async () => {
    ...
    try {
      const data: InvestmentPlan = {
        ...
        minAmount: parseFloat(planMinAmount) || 0,
        maxAmount: parseFloat(planMaxAmount) || 0,
        currency: planCurrency,
        profitRate: parseFloat(planProfitRate) || 0,
  ...
  const handleDeletePlan = async (id: string) => {
    try {
      await remove(ref(database, `adminSettings/investmentPlans/${id}`));
  ...
  const handleCompleteInvestment = async () => {
    ...
    try {
      await update(ref(database, `users/${inv.uid}/investments/${inv.id}`), {
        status: 'completed',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="admin-card border-0 shadow-none">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xl font-bold">{formatNumber(activeInvestments.length)}</p>
              <p className="text-xs text-muted-foreground">استثمارات نشطة</p>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <Card className="admin-card border-0 shadow-none">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xl font-bold">{formatNumber(totalInvested)}</p>
              <p className="text-xs text-muted-foreground">إجمالي المستثمر</p>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="admin-card border-0 shadow-none">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xl font-bold">{formatNumber(totalProfitsPaid)}</p>
              <p className="text-xs text-muted-foreground">أرباح مدفوعة</p>
            </CardContent>
          </Card>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card className="admin-card border-0 shadow-none">
            <CardContent className="p-4 text-center">
              <CheckCircle className="w-6 h-6 mx-auto mb-2 text-teal-500" />
              <p className="text-xl font-bold">{formatNumber(completedInvestments.length)}</p>
      ...
      <Tabs defaultValue="plans">
        ...
        <TabsContent value="plans" className="space-y-4">
          <div className="space-y-3 max-h-[calc(100vh-440px)] overflow-y-auto scrollbar-thin">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {typeLabels[plan.type] || plan.type} - {plan.durationDays} يوم - عائد {plan.profitRate}%
                          </p>
                          <p className="text-xs text-muted-foreground">
                            من {formatNumber(plan.minAmount)} إلى {formatNumber(plan.maxAmount)} {currencySymbols[plan.currency || 'YER']}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge className={plan.isActive ? 'bg-green-500/20 text-green-600 dark:text-green-400' : 'bg-red-500/20 text-red-600 dark:text-red-400'}>
                        ...
                        <Button variant="ghost" size="sm" onClick={() => {
                          ...
                          setPlanMinAmount(String(plan.minAmount));
                          setPlanMaxAmount(String(plan.maxAmount));
                          setPlanCurrency(plan.currency);
                          setPlanProfitRate(String(plan.profitRate));
                          setPlanIsActive(plan.isActive);
                          setDialog(true);
                        }}>
                          <Edit className="w-4 h-4" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => plan.id && handleDeletePlan(plan.id)}>
            ...
            {plans.length === 0 && (
              <p className="text-center text-muted-foreground py-8">لا توجد خطط استثمار</p>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="investments" className="space-y-4">
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="بحث بالاسم أو الخطة..." value={search} onChange={(e) => setSearch(e.target.value)} className="pr-10" />
            </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">عائد: {inv.profitRate}% | ربح: {formatNumber(inv.totalProfit || 0)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {inv.status === 'active' && (
                          <div className="flex gap-1">
                            <Button variant="ghost" size="sm" className="text-green-600" onClick={() => setConfirmDialog({ type: 'complete', investment: inv })}>
                              <CheckCircle className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="sm" className="text-red-600" onClick={() => setConfirmDialog({ type: 'cancel', investment: inv })}>
        ...
        <TabsContent value="settings" className="space-y-4">
          <Card className="admin-card border-0 shadow-none">
            <CardHeader>
              <CardTitle className="text-base">إعدادات الاستثمار</CardTitle>
            </CardHeader>
      ...
      <Dialog open={dialog} onOpenChange={(open) => { setDialog(open); if (!open) resetForm(); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? 'تعديل خطة الاستثمار' : 'إضافة خطة استثمار'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>اسم الخطة</Label>
              <Input value={planName} onChange={(e) => setPlanName(e.target.value)} placeholder="مثال: خطة الاستثمار الذهبي" />
            </div>
            <div>
              <Label>نوع الخطة</Label>
              <Select value={planType} onValueChange={(v: any) => {
                setPlanType(v);
                const durations: Record<string, string> = { daily: '30', weekly: '28', monthly: '30', quarterly: '90' };
                setPlanDuration(durations[v] || '30');
              }}>
            ...
            <div>
              ...
              <Input type="number" value={planDuration} onChange={(e) => setPlanDuration(e.target.value)} dir="ltr" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>الحد الأدنى للمبلغ</Label>
                <Input type="number" value={planMinAmount} onChange={(e) => setPlanMinAmount(e.target.value)} dir="ltr" />
              </div>
              <div>
                <Label>الحد الأقصى للمبلغ</Label>
                <Input type="number" value={planMaxAmount} onChange={(e) => setPlanMaxAmount(e.target.value)} dir="ltr" />
              </div>
            </div>
            <div>
              <Label>العملة</Label>
            ...
            <div>
              ...
              <Input type="number" value={planProfitRate} onChange={(e) => setPlanProfitRate(e.target.value)} placeholder="مثال: 5" dir="ltr" />
            </div>
            <div className="flex items-center gap-2">
      ...
      <AlertDialog open={!!confirmDialog.investment} onOpenChange={(open) => { if (!open) setConfirmDialog({ type: 'complete', investment: null }); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{confirmDialog.type === 'complete' ? 'إكمال الاستثمار' : 'إلغاء الاستثمار'}</AlertDialogTitle>
          </AlertDialogHeader>
          <p className="text-sm text-muted-foreground">
            {confirmDialog.type === 'complete'
              ? 'هل أنت متأكد من إكمال هذا الاستثمار؟ سيتم تحويل الأرباح إلى رصيد المستخدم.'
              : 'هل أنت متأكد من إلغاء هذا الاستثمار؟ سيتم إرجاع المبلغ الأساسي للمستخدم.'}
          </p>
          <AlertDialogFooter>
            <AlertDialogCancel>إلغاء</AlertDialogCancel>
            <AlertDialogAction
              onClick={confirmDialog.type === 'complete' ? handleCompleteInvestment : handleCancelInvestment}
              className={confirmDialog.type === 'complete' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}
            >
