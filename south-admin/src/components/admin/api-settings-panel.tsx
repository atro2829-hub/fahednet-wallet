...
function ExchangeRateSettings() {
  ...
  if (loading) return <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-[#8B1E3A]" /></div>;
        ...
        <Card className="admin-card border-0 shadow-none">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <DollarSign className="w-5 h-5 text-[#8B1E3A]" /> إعدادات أسعار الصرف
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button onClick={handleSaveExchangeRateSettings} disabled={saving} className="w-full bg-[#7B1A30] hover:bg-[#5C1225]">
    ...
    <div className="space-y-4">
      ...
      <Card className="admin-card border-0 shadow-none">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Globe className="w-5 h-5 text-[#8B1E3A]" /> معلومات المزود
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
      ...
      <Card className="admin-card border-0 shadow-none">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Layers className="w-5 h-5 text-[#8B1E3A]" /> قسم الخدمات
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
      ...
      <Card className="admin-card border-0 shadow-none">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            <Code className="w-5 h-5 text-[#8B1E3A]" /> إعدادات الطلب
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {/* Advanced Headers */}
          <button
            ...
            className="flex items-center gap-2 text-sm font-medium text-[#8B1E3A] hover:text-[#A82850]"
          >
                ...
                <div className="mb-3">
                  ...
                  <div className="flex flex-wrap gap-1 max-h-32 overflow-y-auto">
                    {testResult.availableFields.map(field => (
                      <Badge key={field} variant="outline" className="text-[10px] cursor-pointer hover:bg-[#8B1E3A]/10" title="انقر لتعيين كحقل">
                        {field}
                      </Badge>
                    ))}
                  </div>
        ...
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2 justify-between">
            <span className="flex items-center gap-2">
              <ArrowRight className="w-5 h-5 text-[#8B1E3A]" /> تعيين الحقول
            </span>
            <button onClick={() => setShowMappings(!showMappings)} className="text-sm text-[#8B1E3A]">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {testResult.availableFields.slice(0, 10).map(field => (
                              <button
                                ...
                                className="text-[9px] px-1.5 py-0.5 rounded border border-border hover:bg-[#8B1E3A]/10 hover:border-[#8B1E3A]/30 transition-colors"
                              >
                                {field.split('.').pop()}
                              </button>
                            ))}
      ...
      <div className="flex gap-2">
        <Button onClick={handleSave} disabled={!name || !baseUrl} className="flex-1 bg-[#7B1A30] hover:bg-[#5C1225]">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                  <Card className="admin-card border-0 shadow-none">
                    <CardContent className="p-4 text-center">
                      <Code className="w-6 h-6 mx-auto mb-2 text-[#8B1E3A]" />
                    ...
                    <motion.div key={provider.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}>
                      <Card className="admin-card border-0 shadow-none">
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ backgroundColor: 'rgba(168,40,80,0.1)' }}>
                                <Code className="w-5 h-5 text-[#8B1E3A]" />
                              </div>
                              <div>
