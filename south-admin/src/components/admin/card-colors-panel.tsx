...
import { Palette, RotateCcw, Save, CreditCard } from 'lucide-react';
...
export default function CardColorsPanel() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast('تم حفظ ألوان البطائق', 'success');
    } catch (e) {
  ...
  const handleResetCurrency = (currency: keyof CardColors) => {
    setColors((prev) => ({
      ...prev,
      [currency]: defaultColors[currency],
    }));
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">تخصيص ألوان بطائق المحفظة لكل عملة</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleReset} size="sm">
            <RotateCcw className="w-4 h-4 ml-1" /> إعادة تعيين الكل
          </Button>
          ...
          <motion.div key={currency} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
            <Card className="admin-card border-0 shadow-none overflow-hidden">
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${colors[currency].gradientStart}, ${colors[currency].gradientEnd})`,
                }}
              >
              ...
              <CardContent className="p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleResetCurrency(currency)}>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Input
                        type="color"
                        value={colors[currency].primary}
                        onChange={(e) => handleColorChange(currency, 'primary', e.target.value)}
                  ...
                  <div>
                    <Label className="text-xs">بداية التدرج</Label>
                    <div className="flex items-center gap-2 mt-1">
                      <Input
                        type="color"
                        value={colors[currency].gradientStart}
                        onChange={(e) => handleColorChange(currency, 'gradientStart', e.target.value)}
                        className="w-12 h-8 p-1 cursor-pointer"
                      />
                      <Input
                        value={colors[currency].gradientStart}
                  ...
                  <div>
                    <Label className="text-xs">نهاية التدرج</Label>
                    <div className="flex items-center gap-2 mt-1">
                      <Input
                        type="color"
                        value={colors[currency].gradientEnd}
                        onChange={(e) => handleColorChange(currency, 'gradientEnd', e.target.value)}
                        className="w-12 h-8 p-1 cursor-pointer"
                      />
                      <Input
                        value={colors[currency].gradientEnd}
