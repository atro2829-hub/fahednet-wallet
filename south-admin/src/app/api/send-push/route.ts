...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = {
      ...
      webpush: {
        notification: {
          ...
          tag: `south-admin-${type || 'info'}-${Date.now()}`,
        },
        fcm_options: {
          link: data?.url || '/',
        },
    ...
    console.log(`Admin FCM Push: ${response.successCount} success, ${response.failureCount} failed out of ${tokens.length} tokens`);
    ...
    console.error('Admin FCM Push API error:', error);
    return NextResponse.json(
