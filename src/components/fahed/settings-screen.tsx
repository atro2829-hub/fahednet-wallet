...
const settingsSections: SettingsSection[] = [
  {
    ...
    iconColor: '#8B1E3A',
    items: [
      { id: 'my-account', label: 'حسابي', icon: User, color: '#8B1E3A', screen: 'edit-profile' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'change-password', label: 'تغيير كلمة المرور', icon: Shield, color: '#8B1E3A' },
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'advanced-security', label: 'الأمان المتقدم', icon: ShieldCheck, color: '#8B1E3A' },
    ...
    >
      ...
      >
        ...
        <div className="px-5 pb-8 overflow-y-auto max-h-[70vh] space-y-3">
            ...
            <div className="flex gap-2 px-4 pb-3">
              {[
                ...
                <button
                  ...
                  style={{
                    background: language === lang.key ? 'rgba(139,30,58,0.1)' : isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)',
                    border: language === lang.key ? '1.5px solid #8B1E3A' : `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`,
                    color: language === lang.key ? '#8B1E3A' : isDark ? '#AAA' : '#666',
                  }}
                >
                  {language === lang.key && <Check size={14} strokeWidth={2} color="#8B1E3A" />}
                  {lang.label}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={sectionStyle}>
            <div className="flex items-center gap-3 px-4 py-3.5">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(139,30,58,0.12)' }}>
            ...
            <div className="flex gap-2 px-4 pb-3">
              {(['YER', 'SAR', 'USD'] as const).map((cur) => (
                <button
                  ...
                  style={{
                    background: currency === cur ? 'rgba(139,30,58,0.1)' : isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)',
                    border: currency === cur ? '1.5px solid #8B1E3A' : `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'}`,
                    color: currency === cur ? '#8B1E3A' : isDark ? '#AAA' : '#666',
                  }}
                >
                  {currency === cur && <Check size={14} strokeWidth={2} color="#8B1E3A" />}
                  {cur}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-2xl overflow-hidden" style={sectionStyle}>
            <div className="flex items-center gap-3 px-4 py-3.5">
              ...
              <button
                ...
                className="w-11 h-6 rounded-full flex items-center transition-all duration-200 px-0.5"
                style={{
                  background: notifications ? '#8B1E3A' : (isDark ? '#333' : '#DDD'),
        ...
        <div className="px-5 pb-8 space-y-2">
          {(['small', 'medium', 'large'] as const).map((size) => (
            <button
              ...
              style={{
                background: fontSize === size ? 'rgba(139,30,58,0.08)' : isDark ? '#1A1A1A' : '#FFFFFF',
                border: fontSize === size ? '1.5px solid #8B1E3A' : `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.05)'}`,
              }}
            >
              <div className="flex-1 text-right">
                <span className="text-sm font-medium" style={{ color: fontSize === size ? '#8B1E3A' : isDark ? '#DDD' : '#444' }}>{labels[size]}</span>
              </div>
              {fontSize === size && <Check size={18} strokeWidth={2} color="#8B1E3A" />}
            </button>
          ))}
        </div>
      </motion.div>
        ...
        {toastMsg && (
          ...
          >
            <div className="px-4 py-3 rounded-2xl flex items-center gap-2 shadow-lg" style={{
              background: toastMsg.type === 'success' ? 'rgba(16,185,129,0.95)' : 'rgba(139,30,58,0.95)',
              color: '#FFF',
            }}>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {isBiometricItem && biometricStatus && (
                              <span className="text-[10px] block text-right" style={{ color: biometricStatus.available ? '#10B981' : '#8B1E3A' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="px-4 mt-4">
        ...
        >
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(139,30,58,0.08)' }}>
            <Trash2 size={18} strokeWidth={1.5} color="#8B1E3A" />
          </div>
          <span className="flex-1 text-right text-sm font-bold" style={{ color: '#8B1E3A' }}>
            حذف حسابي نهائياً
          </span>
        </button>
      </motion.div>
